...
describe("createDeleteWalletService", () => {
  it("deletes wallet and its challenges for the owner", async () => {
    ...
    const clearCachedPortfolioDataForUser = vi.fn(async () => {});

    const deleteWalletForUser = createDeleteWalletService({
      findWalletById,
      deleteChallengesForWallet,
      deleteWalletById,
      clearCachedPortfolioDataForUser
    });
    ...
    expect(clearCachedPortfolioDataForUser).toHaveBeenCalledWith("u1");
  });
  ...
  it("returns 403 when wallet belongs to a different user", async () => {
    ...
    expect(clearCachedPortfolioDataForUser).not.toHaveBeenCalled();
  });

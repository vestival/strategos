...
### Fixed
- Wallet verification fallback is now resilient to indexer propagation delays: note-based verification retries for a short window and applies a small post-expiry grace period before declaring failure, reducing false `Signed transaction could not be submitted or found on-chain yet` errors when linking additional wallets (`src/lib/verification/verify.ts`, `tests/verify-note.test.ts`) (2026-02-26 14:23 CET)
- Wallet Management no longer falsely appears empty when wallet-list requests fail under transient API issues. `GET /api/wallets/list` is now session-authenticated without list throttling, and the wallets UI now surfaces API errors instead of showing a misleading `No linked wallets yet` state (`src/app/api/wallets/list/route.ts`, `src/app/wallets/page.tsx`, `locales/en.json`, `locales/es.json`) (2026-02-25 16:10 CET)
- Wallet verification no longer reports a misleading `Invalid signed transaction payload` when decoding succeeds but submission fails. The verify route now separates decode/validation from broadcast errors and falls back to on-chain note verification if broadcast fails (for example, when already submitted), returning a specific error only when neither path confirms the transaction (`src/app/api/wallets/verify/route.ts`) (2026-02-24 19:46 MST)
- Wallet verification now rejects unsigned `txn/transaction` wrapper fields when extracting Pera payloads, validates candidate blobs as decodable signed transactions before submit, and accepts base64url variants server-side; this fixes repeated `Invalid signed transaction payload` failures when linking additional wallets (`src/lib/wallet/signed-payload.ts`, `src/app/wallets/page.tsx`, `src/app/api/wallets/verify/route.ts`, `tests/signed-payload.test.ts`) (2026-02-24 19:33 MST)
- Wallet linking is now resilient to Pera/WalletConnect signed-payload shape differences (nested arrays/object wrappers/base64/hex), preventing `Invalid signed transaction payload` errors when linking additional wallets (e.g. a 3rd wallet in the same session) (`src/lib/wallet/signed-payload.ts`, `src/app/wallets/page.tsx`, `tests/signed-payload.test.ts`) (2026-02-24 17:12 MST)
- Added explicit default ASA→CoinGecko mapping for X-NFT (`ASA 1164556102`) so it resolves through primary spot-price flow instead of relying exclusively on fallback sources; includes regression coverage for mapped quote resolution (`src/lib/price/provider.ts`, `tests/price-provider.test.ts`) (2026-02-22 16:48 MST)
...
### Fixed
- Improved light-mode header/settings contrast: user-menu avatar initial now uses high-contrast styling, and the Settings helper pill (`Theme toggle available in header`) now adapts correctly for light/dark themes (2026-02-20 09:15 MST)
- Wallet Analytics chart now normalizes series to UTC daily close points and applies the same day-window behavior as Overview for 7D/30D/90D; aggregate USD view reuses the Overview-style chart renderer for consistent interaction/visualization (2026-02-20 09:15 MST)
- Portfolio history now enforces strict daily points for range views (7D/30D/90D use last N daily points), and for current UTC day it uses latest refresh timestamp/value instead of forced EOD close (2026-02-19 18:36 MST)
